...
RRTStar::RRTStar(const std::pair<float, float> &start_point,
                 ...
                 float map_width,
                 float map_height,
                 std::vector<std::pair<float, float>> roi) : start_point_(start_point),
  ...
  // Set range
  if (roi.size() > 0){
      random_double_.setRange(0, roi.size());
...
bool RRTStar::pathPlanning(std::list<std::pair<float, float>> &path) {
  ...
  while (nodes_.size() < max_num_nodes_) {
    ...
    while (!found_next) {
      if (roi.size() > 0){
      p_rand = sampleUniform();  // roi heuristic
    ...
    if (goal_reached_ ) {
      ...
      std::cout << "-----> Path found, #nodes:" << nodes_.size() << std::endl;
...
std::pair<float, float> RRTStar::sampleUniform() {
  //std::cout << "-- sampleUniform:" << std::endl;

  std::pair<float, float> random_point;
  int idx = random_double_.generate();

  random_point = roi[idx];
  //std::cout << "-- sampleUniform:" << idx  << " => "  << random_point.first << ", " << random_point.second <<std::endl;

  random_double2_.setRange(random_point.first -1, random_point.first +1);

  random_point.first = random_double2_.generate();

  random_double2_.setRange(random_point.second -1, random_point.second +1);
  random_point.second = random_double2_.generate();
